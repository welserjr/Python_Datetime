...
class Quiz:
    ...
    def summary(self):
        # print how many you got right and the total # questions. 5/10
        print("You got {} out of {} right.".format(self.total_correct(), len(self.questions)))
        # print the total time for the quiz: 30 seconds!
        if (self.end_time - self.start_time).seconds > 60:
            min = round((self.end_time - self.start_time).seconds / 60)
            seg = (self.end_time - self.start_time).seconds % 60
            print("It took you {} min and {} seconds total.".format(min, seg))
        else:
